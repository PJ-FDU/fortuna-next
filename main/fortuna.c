...
/*=== 星盘样式参数（NEW） ===*/
#define OUTER_RING_W 3                       /* 外圈宽度：3px */
#define INNER_RING_W 2                       /* 内圈宽度：2px */
#define PLANET_DOT_R 8                       /* 行星点半径：从 4 → 8 */
#define PLANET_SEP_DEG 6.0f                  /* 近邻判定的角距（度） */
#define PLANET_SEP_OFFSET (PLANET_DOT_R + 4) /* 近邻时的径向错位像素 */
                          ...
                          uint16_t dash_w, uint16_t dash_gap)
{
    ...
    if (dash_w && dash_gap)
    {
        ld.dash_width = dash_w;
        ld.dash_gap = dash_gap;
    }
    lv_draw_line(layer, &ld);
}
                            ...
                            lv_color_t color, const lv_font_t *font)
{
    ...
    /* 根据字体行高估算包围盒，宽度放大些以容纳2字符 */
    int fh = lv_font_get_line_height(font);
    int half_w = (int)(fh * 1.4f);
    int half_h = (int)(fh * 0.8f);
...
static void astro_draw_cb(lv_event_t *e)
{
    ...
    /* 1) 外圈双环（修改：使用 OUTER_RING_W / INNER_RING_W） */
    ...
    {
        /* 简易避让：与前一颗小于 6° 时上下错位 */
        static float lastL = -1000.0f;
        static bool flip = false;
        float L = norm360f(BODIES[i].Ldeg);
        float r = (fabsf(L - lastL) < PLANET_SEP_DEG)
                      ? (R_planet + (flip ? -PLANET_SEP_OFFSET : +PLANET_SEP_OFFSET))
                      : R_planet;
        flip = !flip;
        lastL = L;
        ...
        draw_dot(layer, p.x, p.y, PLANET_DOT_R,
                 COL_PNTS[i % (int)(sizeof(COL_PNTS) / sizeof(COL_PNTS[0]))]);
    ...
    for (int i = 0; i < N_BODIES; i++)
    {
        for (int j = i + 1; j < N_BODIES; j++)
        {
            ...
            {
                if (fabsf(d - ASPECTS[k].deg) <= ASPECTS[k].orb)
                {
                    hit = k;
                    break;
                }
